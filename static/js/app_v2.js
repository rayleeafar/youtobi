...
async function loadConfig() {
  try {
    ...
    if (data.success) {
      ...
      if (cfg.llm_api_key) document.getElementById('llm_api_key').value = cfg.llm_api_key;
...
function renderTaskCard(task) {
  ...
  return `
    <div class="task-card">
      ...
      ${task.bvid ? `
        <div style="background: rgba(0, 230, 118, 0.1); border: 1px solid rgba(0, 230, 118, 0.3); padding: 0.8rem 1rem; border-radius: var(--radius-md); margin-bottom: 1rem; color: var(--success);">
          🎉 已成功同步发布至 Bilibili! BV号: <strong>${task.bvid}</strong>
        </div>
      ` : ''}
      ...
      ${task.final_description ? `
        <div style="margin-bottom: 1rem; font-size: 0.9rem; color: var(--text-main); background: rgba(255,255,255,0.03); padding: 0.8rem; border-radius: var(--radius-sm);">
          <strong>B站发布简介 (LLM ${task.used_llm ? '已生成' : '未启用'}):</strong>
